  ...
  return (
    <div className="min-h-full flex items-center pt-32 pb-20 px-6 md:px-12 max-w-[1550px] mx-auto relative group/section">
      ...
      <div className="w-full flex flex-col lg:flex-row gap-12 lg:gap-24 items-start">
        ...
        {/* Left Section: Context & Title & Minister Banner (35%) */}
        <div className="lg:w-[35%] shrink-0 pt-4">
          <div className="mb-12">
            ...
            <p className="text-slate-500 text-[15px] leading-relaxed max-w-[280px] font-medium break-keep mb-12">
              대한민국의 위상을 높이고 국익을 증진시키기 위한 <br />범정부 차원의 주요 외교 현장을 전달합니다.
            </p>
            <a href="#" className="inline-flex items-center gap-2 text-xs font-black text-primary/80 hover:text-gold transition-all group tracking-[0.2em] uppercase mb-16 block">
              View All Activities
              <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </a>

            {/* Minister Banner (Image Reference Style) */}
            <motion.div 
              initial={{ opacity: 0, x: -30 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              className="relative w-full max-w-[420px] aspect-[2/1] bg-gradient-to-r from-blue-700 via-indigo-600 to-blue-500 rounded-[2.5rem] shadow-2xl overflow-hidden flex group/banner"
            >
              <div className="relative z-10 w-[60%] p-6 flex flex-col justify-center">
                <h4 className="flex items-center gap-1.5 text-white font-medium text-sm mb-1 opacity-90">
                  <span className="text-xs">안녕하십니까</span>
                </h4>
                ...
                <p className="text-[10px] text-white/90 leading-relaxed font-bold break-keep mb-6">
                  "엄중한 외교 상황 속에서 <br/>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button className="bg-white text-slate-700 text-[9px] font-black py-2 rounded-full hover:bg-gold transition-colors">장관소개</button>
                  <button className="bg-white text-slate-700 text-[9px] font-black py-2 rounded-full hover:bg-gold transition-colors">장관과의 대화</button>
                  <button className="bg-white text-slate-700 text-[9px] font-black py-2 rounded-full hover:bg-gold transition-colors">주요일정</button>
                  <button className="bg-[#8D6E63] text-white text-[9px] font-black py-2 rounded-full hover:brightness-110 transition-all">장관 외교활동</button>
                </div>
              </div>
              ...
              <div className="w-[40%] h-full relative flex items-end">
                <img 
                  ...
                  alt="Minister" 
                  className="absolute bottom-0 right-[-10px] h-[95%] object-contain drop-shadow-[-10px_0_20px_rgba(0,0,0,0.3)] group-hover/banner:scale-105 transition-transform duration-700" 
                />
              </div>
            </motion.div>
          </div>
          ...
          {ACTIVITIES.slice(0, 4).map((activity, index) => (
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <h3 className="font-bold text-primary text-[20px] lg:text-[23px] leading-tight group-hover:text-gold transition-colors line-clamp-2 break-keep tracking-tight">
                  {activity.title}
                </h3>
